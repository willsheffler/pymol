...
def drawtetra():
   ...
   with open("/work/sheffler/tmp/hex.pdb", 'w') as o:
      for ix, iy, iz in [(float(x), float(y), float(z)) for x in range(N + 0)
...
def drawcells(E, col=(1, 1, 1), CR=0.53):
   ...
   for p, q in E:
      ...
      cen = (p + q) / 2.0
      nrm = (p - q).normalized()
      cgo += cgo_cyl(c1=cen + nrm / 500, c2=cen - nrm / 500, r=sqrt(CR * CR - (dis * dis / 4)),
                     col=col)
   return cgo
...
def drawbcc(N=2, scale=1.0, col=(1, 1, 1)):
   ...
   P, E = list(), list()
   # with open("/work/sheffler/tmp/hex.pdb",'w') as o:
   for ix, iy, iz in [(float(x), float(y), float(z)) for x in range(N + 1) for y in range(N + 1)
                      for z in range(N + 1)]:
...
def drawfcc():
   cmd.delete("*")
   N = 2
   for x, y, z in [(float(x), float(y), float(z)) for x in range(N + 1) for y in range(N + 1)
                   for z in range(N + 1)]:
