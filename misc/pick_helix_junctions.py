...
def get_pdbids():
   ...
   phetero = set()
   pbadman = set()
   with open("/Users/sheffler/dropbox/project/pdbcatalog/biounit_term_helix_ids.txt") as F:
      [thelix.add(l[:4]) for l in F]
   with open("/Users/sheffler/dropbox/project/pdbcatalog/biounit_term_helix_nres.txt") as F:
      [
         psmall.add(l[:4])
         for l in F
         if (int(l.split()[1]) <= MAXCLEN and int(l.split()[2]) <= MAXTLEN)
      ]
   with open("/Users/sheffler/dropbox/project/pdbcatalog/3dcomplex_NPS.txt") as F:
      [pnosym.add(l[:4]) for l in F]
   with open("/Users/sheffler/dropbox/project/pdbcatalog/pisa_heteromers_ids.txt") as F:
      [phetero.add(l[:4]) for l in F]
   with open("/Users/sheffler/dropbox/project/pdbcatalog/biounit_term_helix_badman.txt") as F:
      [pbadman.add(l[:4]) for l in F]
   pids = set(thelix)
...
class TH(object):
   ...
   def show(self, x=Xform()):
      ...
      if not tag in cmd.get_object_list():
         load_biounit(
            "%s/%s/%s.pdb%s.gz" % (BIOUNIT_DIR, self.pdb[1:3], self.pdb[:4], self.pdb[4]), tag)
      ...
      showvecfrompoint(x.R * self.axs * 20, x * self.cen - x.R * self.axs * 10,
                       lbl=self.id + "_HA")
...
class HH(object):
   ...
   def __init__(self, line, hconnect):
      ...
      xty = float(dat[12])
      xtz = float(dat[13])
      self.x = Xform(rotation_matrix_degrees(Vec(rax, ray, raz), float(dat[7])), Vec(
         xtx, xty, xtz))
...
def cachefiles():
   return "/Users/sheffler/tmp/hterm_%03i-%03i.dat.gz" % (
      MAXCLEN, MAXTLEN), "/Users/sheffler/tmp/hjunct_%03i-%03i.dat.gz" % (MAXCLEN, MAXTLEN)
   ...
   if nstates > 1:
      for s in range(1, nstates + 1):
         for c in cindex:
            ...
            cmd.alter(obj + "_STATE_%i_%i" % (s, c),
                      "chain='%s'" % ROSETTA_CHAINS[(s - 1) * nchain + c])
      cmd.delete(obj)
...
def makesym(axs, cen, nf, objs=None, tag="makesym"):
   ...
   for i in range(nf):
      ...
      for j, o in enumerate(tmpobjs):
         cmd.alter(o, "chain='%s'" % ROSETTA_CHAINS[i * len(tmpobjs) + j])
   ...
   util.cbc()
   cmd.zoom()
   print "save /Users/sheffler/project/struts/picks/C%i_" % nf + "_".join(
      objs) + ".pdb," + tag + "_*"
   return True
...
def helix_joint_has_clashes(a, b):
   ...
   if ha.chain_num < 0 and hb.chain_num > 0:
      s1 = "%s and not (chain %s and resi %i-999999)" % (ha.pdb, ha.chain, ha.pdb_bounds()[0] - 4
                                                         )  # FIXME chain A
      s2 = "%s and not (chain %s and resi 000000-%i)" % (hb.pdb, hb.chain, hb.pdb_bounds()[1] + 4
                                                         )  # FIXME chain A
   elif ha.chain_num > 0 and hb.chain_num < 0:
      s1 = "%s and not (chain %s and resi 000000-%i)" % (ha.pdb, ha.chain, ha.pdb_bounds()[1] + 4
                                                         )  # FIXME chain A
      s2 = "%s and not (chain %s and resi %i-999999)" % (hb.pdb, hb.chain, hb.pdb_bounds()[0] - 4
                                                         )  # FIXME chain A
   else:
...
def gen3way(joints, forbid=set(FORBID_DEFAUT), nflb=2, nfub=99999, starti=0):
   ...
   for i in range(starti, len(joints)):
      ...
      for j in range(len(joints)):
         ...
         if abs(a.h1.chain_num) == abs(a.h2.chain_num) and abs(b.h1.chain_num) == abs(
               b.h2.chain_num):
            continue
         ...
         ofsterr = math.sqrt(((x * a.h2.cen - xsym * a.h2.cen).length_squared() +
                              (x * b.h2.cen - xsym * b.h2.cen).length_squared()) / 2.0)
         ...
         fn = "/data/project/helix_assembly/cyclic/C%i_" % nf + "_".join(
            objs) + "_%i_%i.pdb" % (i, j)
         for inf in range(nf):
...
if __name__ == '__main__':
   ...
   ax = Xform(
      Mat(Vec(-0.902341, 0.379216, -0.204879), Vec(-0.184325, -0.769177, -0.611875),
          Vec(-0.389621, -0.514356, 0.763959)), Vec(90.645917, 24.612097, 40.190192))
   bx = Xform(
      Mat(Vec(0.953091, 0.197334, -0.229515), Vec(-0.250173, 0.940406, -0.230325),
          Vec(0.170387, 0.276939, 0.945660)), Vec(1.351979, -1.708957, 21.053139))
   xaln = Xform(
      Mat(Vec(0.568684, -0.797590, -0.201118), Vec(-0.818463, -0.573041, -0.041746),
          Vec(-0.081953, 0.188348, -0.978677)), Vec(-10.364693, 97.473637, 11.653443))
