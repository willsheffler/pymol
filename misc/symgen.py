...
def symgentest(SYM, NFOLD, WHICHRODS=(1, 1, 1), MOVASYM=0.7, SYMGEN=4, SKEW=0.5, SKEW0=0.5):
   ...
   c0, c1, c2 = r0 * SYMAXIS[SYM, NFOLD[0]] + SYMCEN[SYM, NFOLD[0]], r1 * SYMAXIS[
      SYM, NFOLD[1]] + SYMCEN[SYM, NFOLD[1]], r2 * SYMAXIS[SYM, NFOLD[2]] + SYMCEN[SYM, NFOLD[2]]

   bo0, bo1, bo2 = (
      random.uniform(0.05, 0.10) * projperp(v, randvec()).normalized() for v in (c0, c1, c2))
   ...
   if WHICHRODS[0]:
      showcyl(hoa0 + framecen * MOVASYM, hob1 + framecen * MOVASYM, STRUTRAD, col=(1, 0, 0),
              col2=(0, 1, 0), lbl="asym_rod1")
   if WHICHRODS[1]:
      showcyl(hoa1 + framecen * MOVASYM, hob2 + framecen * MOVASYM, STRUTRAD, col=(0, 1, 0),
              col2=(0, 0, 1), lbl="asym_rod2")
   if WHICHRODS[2]:
      showcyl(hoa2 + framecen * MOVASYM, hob0 + framecen * MOVASYM, STRUTRAD, col=(0, 0, 1),
              col2=(1, 0, 0), lbl="asym_rod3")
   ...
   for i, xs in enumerate(expand_xforms((G1, G2), SYMGEN)):
      ...
      if WHICHRODS[0]:
         showcyl(xs * hoa0, xs * (c0), STRUTRAD, col=(1, 0, 0), lbl="sym%02i_cena1" % i)
      if WHICHRODS[1]:
         showcyl(xs * hoa1, xs * (c1), STRUTRAD, col=(0, 1, 0), lbl="sym%02i_cena2" % i)
      if WHICHRODS[2]:
         showcyl(xs * hoa2, xs * (c2), STRUTRAD, col=(0, 0, 1), lbl="sym%02i_cena3" % i)

      if WHICHRODS[2]:
         showcyl(xs * hob0, xs * (c0), STRUTRAD, col=(1, 0, 0), lbl="sym%02i_cenb1" % i)
      if WHICHRODS[0]:
         showcyl(xs * hob1, xs * (c1), STRUTRAD, col=(0, 1, 0), lbl="sym%02i_cenb2" % i)
      if WHICHRODS[1]:
         showcyl(xs * hob2, xs * (c2), STRUTRAD, col=(0, 0, 1), lbl="sym%02i_cenb3" % i)

      if WHICHRODS[0] and WHICHRODS[2]:
         showcyl(xs * hoa0, xs * hob0, STRUTRAD, col=(1, 0, 0), lbl="sym%02i_cenc1" % i)
      if WHICHRODS[1] and WHICHRODS[0]:
         showcyl(xs * hoa1, xs * hob1, STRUTRAD, col=(0, 1, 0), lbl="sym%02i_cenc2" % i)
      if WHICHRODS[2] and WHICHRODS[1]:
         showcyl(xs * hoa2, xs * hob2, STRUTRAD, col=(0, 0, 1), lbl="sym%02i_cenc3" % i)

      if WHICHRODS[0]:
         showcyl(xs * hoa0, xs * hob1, STRUTRAD, col=(1, 0, 0), col2=(0, 1, 0),
                 lbl="sym%02i_rod1" % i)
      if WHICHRODS[1]:
         showcyl(xs * hoa1, xs * hob2, STRUTRAD, col=(0, 1, 0), col2=(0, 0, 1),
                 lbl="sym%02i_rod2" % i)
      if WHICHRODS[2]:
         showcyl(xs * hoa2, xs * hob0, STRUTRAD, col=(0, 0, 1), col2=(1, 0, 0),
                 lbl="sym%02i_rod3" % i)
